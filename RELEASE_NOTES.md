...
- **`delete_pool` refuses `.mgr` and `.rgw.root`**, and any pool whose application metadata marks it a mgr pool. Deleting `.mgr` breaks ceph-mgr, which serves the Dashboard API this tool's every call depends on.
