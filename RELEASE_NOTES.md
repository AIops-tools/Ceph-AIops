...
- **`set_pool_size` refuses shrinking `.mgr` / `.rgw.root`** — dropping the mgr pool to size 1 degrades ceph-mgr, which serves the Dashboard API this tool depends on. An increase is allowed.
