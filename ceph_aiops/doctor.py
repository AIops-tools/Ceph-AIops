"""Environment and connectivity diagnostics for Ceph AIops."""

from __future__ import annotations

from rich.console import Console

from ceph_aiops.config import CONFIG_FILE, ENV_FILE, load_config
from ceph_aiops.secretstore import SECRETS_FILE, check_permissions, has_store
...
def run_doctor(skip_auth: bool = False) -> int:
    ...
    if not CONFIG_FILE.exists():
        ...
        _console.print("[yellow]  Run 'ceph-aiops init' to set up your first target.[/]")
        return 1
    ...
    if has_store():
        ...
        _console.print(
            ...
            f"'ceph-aiops secret migrate'.[/]"
        )
    else:
        _console.print(
            "[yellow]! No secret store yet. Run 'ceph-aiops init' to set up "
    ...
    for target in config.targets:
        try:
            ...
            # Health probe doubles as a login + mgr-dashboard reachability check.
            info = conn.get("/api/health/minimal")
            health = info.get("health") if isinstance(info, dict) else None
            status = (health or {}).get("status", "?")
            _console.print(
                f"[green]✓ Connected to '{target.name}' ({target.host}:{target.port}) "
                f"— Ceph mgr dashboard reachable, cluster {status}[/]"
            )
