"""``ceph-aiops secret`` — manage the encrypted credential store.

Secrets (Ceph API keys) are stored in ``~/.ceph-aiops/secrets.enc``
...
from ceph_aiops.config import SECRET_ENV_PREFIX, SECRET_ENV_SUFFIX
from ceph_aiops.secretstore import (
    SECRETS_FILE,
    SecretStore,
    check_permissions,
    migrate_legacy_env,
...
def secret_list() -> None:
    ...
    names = store.names()
    if not names:
        console.print("[yellow]No secrets stored yet. Add one: ceph-aiops secret set <name>[/]")
        return
...
def secret_rotate_password() -> None:
    ...
    console.print("[green]✓ Master password rotated. Update CEPH_AIOPS_MASTER_PASSWORD.[/]")
