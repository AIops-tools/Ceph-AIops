"""Policy engine — rule-based access control for Ceph MCP tools.

Rules are loaded from ``~/.ceph-aiops/rules.yaml`` with hot-reload on file change.
"""
...
_log = logging.getLogger("ceph-aiops.policy")
...
class PolicyEngine:
    ...
    def check_allowed(
        ...
        *,
        ...
        if os.environ.get("CEPH_POLICY_DISABLED") == "1":
            ...
            if not self._rules and risk_level in ("high", "critical"):
                return TierDecision(
                    ...
                    reason=(
                        ...
                        "require a named approver (CEPH_AUDIT_APPROVED_BY) by "
