...
2. **Runaway breaker (on by default).** The same ``(tool, params)`` called more
   than ``CEPH_RUNAWAY_MAX`` times within ``CEPH_RUNAWAY_WINDOW_SEC`` trips a
...
from ceph_aiops.governance.policy import PolicyResult
...
class BudgetExceeded(Exception):
    ...
    Subclasses nothing Ceph-specific; the decorator wraps it into a
