"""Encrypted secret storage for ceph-aiops.

Secrets (Ceph API keys) are NEVER written to disk in plaintext. They
live in ``~/.ceph-aiops/secrets.enc`` encrypted with Fernet (AES-128-CBC +
...
The master password is resolved (in order) from:
  ...
  1. the env var ``CEPH_AIOPS_MASTER_PASSWORD`` (for non-interactive use:
...
from ceph_aiops.governance.paths import ops_home

# ─── Tool-specific constants (change these three to vendor for another tool) ──
APP_NAME = "ceph-aiops"
CONFIG_DIR = ops_home()
MASTER_PASSWORD_ENV = "CEPH_AIOPS_MASTER_PASSWORD"  # nosec B105 — env-var name
# ──────────────────────────────────────────────────────────────────────────────
...
def migrate_legacy_env(prefix: str, suffix: str, password: str | None = None) -> list[str]:
    ...
    Legacy keys look like ``CEPH_<TARGET>_PASSWORD``; ``prefix='CEPH_'`` and
