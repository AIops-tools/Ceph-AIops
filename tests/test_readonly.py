...
from ceph_aiops.governance import READ_ONLY_ENV, is_read_only
...
def test_env_var_is_this_tools_prefix():
    """Brand isolation: ceph-aiops must not read another tool's switch."""
    assert READ_ONLY_ENV == "CEPH_READ_ONLY"
...
def test_governed_write_is_refused_in_read_only(read_only):
    """A write is refused before it can reach the Dashboard — from any caller."""
    calls = []

    @governed_tool(risk_level="high")
    def purge_everything(osd_id: int = 0) -> dict:
        calls.append(osd_id)
        return {"purged": True}
...
def test_governed_read_still_works_in_read_only(read_only):
    ...
    def list_osds(target: str = "lab") -> list:
        return [{"id": 0}]

    assert list_osds() == [{"id": 0}]


@pytest.mark.unit
def test_medium_risk_write_is_also_refused(read_only):
    """Only 'low' is a read — medium (e.g. trigger_scrub) must be refused too."""

    @governed_tool(risk_level="medium")
    def scrub_a_pg(pgid: str = "2.1a") -> dict:
...
def test_apply_read_only_unregisters_write_tools(monkeypatch):
    ...
    try:
        ...
        assert "osd_purge" in dropped and "pool_delete" in dropped
        # Scrub triggers and flag-set are writes too, however benign.
        assert "trigger_scrub" in dropped and "cluster_flag_set" in dropped
        assert "osd_tree" not in dropped and "cluster_health" not in dropped
        ...
        assert "osd_tree" in remaining, "reads must still be exposed"
    finally:
        registry.clear()
