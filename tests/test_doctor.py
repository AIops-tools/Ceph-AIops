"""Tests for ``ceph_aiops.doctor.run_doctor``.

Everything runs against an ``isolated_home`` (see conftest) — no real
``~/.ceph-aiops`` and no network: the connectivity check is exercised by
...
LAB1 = {"name": "lab1", "host": "ceph.example.com", "port": 8443, "username": "admin"}
...
def test_no_secret_store_and_no_password(isolated_home, doctor_out):
    ...
    assert "✗ No password for target 'lab1'" in out


def test_legacy_env_file_warns_but_works(isolated_home, doctor_out, monkeypatch):
    _write_config(isolated_home, [LAB1])
    (isolated_home / ".env").write_text("CEPH_LAB1_PASSWORD=legacy\n", "utf-8")
    monkeypatch.setenv("CEPH_LAB1_PASSWORD", "legacy")
    assert doc.run_doctor(skip_auth=True) == 0
    out = doctor_out.getvalue()
    assert "legacy plaintext .env" in out
    assert "secret migrate" in out
    assert "✓ Password present for 'lab1'" in out
...
class _FakeConn:
    def __init__(self, health: dict | Exception) -> None:
        self._health = health

    def get(self, path: str, **_: Any) -> dict:
        if isinstance(self._health, Exception):
            raise self._health
        assert path == "/api/health/minimal"
        return self._health
...
def test_healthy_end_to_end(isolated_home, doctor_out, fake_mgr):
    _write_config(isolated_home, [LAB1])
    _seed_secret("lab1", "s3cret")
    fake_mgr.results["lab1"] = {"health": {"status": "HEALTH_OK"}}
    assert doc.run_doctor() == 0
    out = doctor_out.getvalue()
    assert "✓ Connected to 'lab1' (ceph.example.com:8443)" in out
    assert "Ceph mgr dashboard reachable, cluster HEALTH_OK" in out
...
def test_connect_failure_is_status_not_crash(isolated_home, doctor_out, fake_mgr):
    ...
    assert doc.run_doctor() == 1
    assert "✗ Connect to 'lab1' failed: connection refused" in doctor_out.getvalue()


def test_mixed_fleet_one_bad_target_fails_overall(isolated_home, doctor_out, fake_mgr):
    lab2 = {**LAB1, "name": "lab2", "host": "ceph2.example.com"}
    _write_config(isolated_home, [LAB1, lab2])
    _seed_secret("lab1", "s1")
    _seed_secret("lab2", "s2")
    fake_mgr.results["lab1"] = {"health": {"status": "HEALTH_WARN"}}
    fake_mgr.results["lab2"] = RuntimeError("401 auth failed")
    assert doc.run_doctor() == 1
    out = doctor_out.getvalue()
    assert "✓ Connected to 'lab1'" in out
    assert "cluster HEALTH_WARN" in out
    assert "✗ Connect to 'lab2' failed: 401 auth failed" in out
