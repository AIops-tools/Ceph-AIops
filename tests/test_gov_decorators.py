...
``ceph_aiops.governance.*`` and binds state to an isolated home via
``CEPH_AIOPS_HOME``. Every governed function under test is SYNTHETIC and
...
import ceph_aiops.governance.budget as budget_mod
import ceph_aiops.governance.decorators as dec_mod
...
from ceph_aiops.governance import PolicyDenied, governed_tool
from ceph_aiops.governance.budget import BudgetExceeded
...
def dec_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("CEPH_POLICY_DISABLED", raising=False)
    monkeypatch.delenv("CEPH_MAX_TOOL_CALLS", raising=False)
    monkeypatch.delenv("CEPH_MAX_TOOL_SECONDS", raising=False)
    _reset()
    yield tmp_path
    _reset()
...
def test_budget_exceeded_propagates_and_audits(dec_home, monkeypatch):
    monkeypatch.setenv("CEPH_MAX_TOOL_CALLS", "0")
...
def test_policy_disabled_marks_status_bypassed(dec_home, monkeypatch):
    monkeypatch.setenv("CEPH_POLICY_DISABLED", "1")
    _reset()
