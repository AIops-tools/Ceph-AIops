...
throwaway home (``CEPH_AIOPS_HOME``) and assert that the rows compliance
...
def gov_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.delenv("CEPH_AUDIT_APPROVED_BY", raising=False)
...
def test_high_risk_allowed_with_named_approver(gov_home, monkeypatch):
    monkeypatch.setenv("CEPH_AUDIT_APPROVED_BY", "storage-alice")
...
def test_real_write_tool_persists_priorstate_undo(gov_home, monkeypatch):
    """End-to-end through a REAL product write tool: osd_reweight must capture
    the OSD's prior weight before changing it and persist the restore-undo on
    disk."""
    monkeypatch.setenv("CEPH_AUDIT_APPROVED_BY", "storage-alice")
    ...
    result = gov.osd_reweight(osd_id=3, weight=0.0)
    assert result["priorState"]["weight"] == 1.0
    assert result.get("_undo_id")

    undo = _rows(gov_home / "undo.db", "undo_log")
    assert len(undo) == 1
    assert undo[0]["undo_tool"] == "osd_reweight"
    assert json.loads(undo[0]["undo_params"]) == {"osd_id": 3, "weight": 1.0}

    audit = _rows(gov_home / "audit.db", "audit_log")
    assert [r["tool"] for r in audit] == ["osd_reweight"]
